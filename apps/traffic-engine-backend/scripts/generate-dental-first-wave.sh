#!/usr/bin/env bash
# Phase 6 orchestrator: seed templates/subjects, then generate first-wave dental care content.
#
# Usage:
#   BASE_URL=https://nestino-backend-production.up.railway.app \
#   ADMIN_PASSWORD=... \
#   ./scripts/generate-dental-first-wave.sh
#
...
log() { printf '[dental-first-wave] %s\n' "$*" >&2; }
...
if [[ "${SKIP_SEED:-false}" != "true" ]]; then
  ...
  bash "${SCRIPT_DIR}/seed-dental.sh"
else
  log "Step 1/3: Skipped seed (SKIP_SEED=true)"
fi

log "Step 2/3: Generating country guides + treatment page..."
bash "${SCRIPT_DIR}/generate-dental-country-guides-direct.sh"

if [[ "${SKIP_CITY_GUIDES:-false}" != "true" ]]; then
  log "Step 3/3: Generating city guides..."
  bash "${SCRIPT_DIR}/generate-dental-city-guides-direct.sh" --all
else
  log "Step 3/3: Skipped city guides (SKIP_CITY_GUIDES=true)"
fi

log "First wave complete. Run generate-dental-phase2-content.sh for cost, origin, and article pages."

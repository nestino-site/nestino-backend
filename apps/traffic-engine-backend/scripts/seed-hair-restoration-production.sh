...
ensure_treatment() {
  ...
  local exists
  exists=$(echo "$raw" | python3 -c "import json,sys;d=json.load(sys.stdin);codes=[t['code'] for t in d];print('yes' if 'HAIR_RESTORATION' in codes else 'no')" 2>/dev/null || echo "no")
  if [[ "$exists" == "yes" ]]; then
...
wait_target() {
  ...
  while [[ $(date +%s) -lt $deadline ]]; do
    sleep "$POLL"
    local count
    count=$(hair_count "$cid" || echo "$last")
    [[ "$count" =~ ^[0-9]+$ ]] || count="$last"
    if [[ "$count" != "$last" ]]; then
      log_file "    hair clinics: $count/$target"
      last="$count"
      stale=0
    else
      stale=$((stale + 1))
  ...
  hair_count "$cid" || echo "$last"
}

# Keyword passes — FUE is most reliable; hair transplant often INVALID_REQUEST on Google.
PASS_KEYWORDS=(
  '["FUE"]'
  '["trasplante capilar"]'
  '["hair transplant"]'
